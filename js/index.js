...
window.onload = () =>{  //onload event is fired when the entire page loads, including its content(imgs/css/scripts/etc) 
...
/* ---------------------------------------------
        Toggle the pop up list display
------------------------------------------------*/
function listOption(event) {
  ...
  if(listMenuPopup.style.display == "block") {
    ...
    let btn = event.target;  // event.target ka <i> pyit nay lo 
    ...
    const loc = btn.getBoundingClientRect();      //returns the size of an element and its position relative to the viewport.
    // console.log(loc);

    listMenuPopup.style.top = loc.top + loc.height + 5 + "px"; //loc.top = a paw page ka nay button yk head hti distance
    // loc.height = button yk height 
    //PopUpMenu yk top ka nay position ko button yk out mr paw say chin lo loc top nk loc height ko pgg dr 
...
/* ---------------------------------------------
        add another list toggle display 
------------------------------------------------*/
...
function saveNewList() {
  ...
  if(listTitle) {
    ...
    .then(data => {
      ...
      // insert it before <add another list>
